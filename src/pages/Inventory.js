...
function Inventory() {
  ...
  const currentRecords = filteredInventory.slice(
    indexOfFirstRecord,
    indexOfLastRecord
  );
  ...
  const handleSave = async () => {
    try {
      if (currentItem._id) {
        // EDIT
        await axios.put(
          `${process.env.REACT_APP_API_URL}/api/inventory/${currentItem._id}`,
          currentItem
        );
      } else {
        // ADD
  ...
  const confirmDelete = async () => {
    try {
      await axios.delete(
        `${process.env.REACT_APP_API_URL}/api/inventory/${deleteTargetId}`
      );
  ...
  const handleMultiAdd = async () => {
    ...
    try {
      for (let i = 0; i < lines.length; i++) {
        const line = lines[i];
        const [id, name, type, status, dateOfPurchase, remarks] = line
          .split(";")
          .map((f) => f.trim());
        const item = {
          id: Number(id),
    ...
    <div style={{ padding: "20px" }}>
      ...
      <div className="d-flex justify-content-between align-items-center mt-3">
        ...
        <div className="d-flex flex-wrap">
          {[
            {
              label: "Slim",
              selectedColor: "#225ffbff",
              borderColor: "#102968ff",
              defaultColor: "#143a98ff",
            },
            {
              label: "Round",
              selectedColor: "#00aaffff",
              borderColor: "#00486dff",
              defaultColor: "#006ea6ff",
            },
            {
              label: "Dispenser",
              selectedColor: "#00dfefff",
              borderColor: "#005359ff",
              defaultColor: "#0096a1ff",
            },
          ].map((type) => {
      ...
      <Modal show={showModal} onHide={() => setShowModal(false)}>
        <Modal.Header closeButton>
          <Modal.Title>
            {currentItem._id ? "Edit Inventory" : "Add Inventory"}
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <Form>
            <Form.Group>
              ...
              <Form.Control
                ...
                onChange={(e) =>
                  setCurrentItem({ ...currentItem, id: Number(e.target.value) })
                }
              />
            </Form.Group>
            <Form.Group>
                ...
                {[
                  ...
                  return (
                    <Button
                      ...
                      className="me-2 mb-2"
                      onClick={() =>
                        setCurrentItem({ ...currentItem, type: type.label })
                      }
                      style={{
                        backgroundColor:
                          type.bg === "navy blue" && isSelected
                            ? "#2b35c4ff"
                            : type.bg === "navy blue"
                            ? "transparent"
                            : undefined,
                        borderColor:
                          type.bg === "navy blue" ? "#1b206bff" : undefined,
                        color:
                          type.bg === "navy blue" && isSelected
                            ? "#636dfeff"
                            : type.bg === "purple"
      ...
      >
        ...
        <Modal.Body>
          <Form>
            <Form.Group>
              <Form.Label>
                Enter multiple inventory records (one per line,
                semicolon-separated fields:
                id;name;type;status;dateOfPurchase;remarks)
              </Form.Label>
              <Form.Control
                as="textarea"
                rows={10}
          ...
          {importing && (
            <div className="mt-3">
              ...
              <div className="text-center mt-2">
                Importing... DO NOT CLOSE. Please wait.
              </div>
            </div>
          )}
      ...
      >
        ...
        <Modal.Body>
          Are you sure you want to delete {selectedIds.length} selected item
